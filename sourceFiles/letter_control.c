...
void draw_A(void) {
	...
	if (pos_aktiv){
		...
		USART_POSITIONIERUNG_X(1);}				//sende Anfangs Position, wenn Positionierung in GUI aktiv
	move_pen_forward();							//setzte Stift vor
	if (pos_aktiv){USART_POSITIONIERUNG_X(1);}	//sendet aktuelle Position, wenn Positionierung in GUI aktiv ist
...
void draw_B(void) {
	...
	move_pen_forward();
	if (pos_aktiv) USART_POSITIONIERUNG_X(1);
